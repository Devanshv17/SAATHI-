...
export const Compare: React.FC<CompareProps> = ({ gameTitle, isHindi }) => {
  const { user } = useAuth();
  ...
  const [leftAssets, setLeftAssets] = useState<string[]>([]);
  const [rightAssets, setRightAssets] = useState<string[]>([]);
  ...
  const initQuestionState = (qs: any[], idx: number) => {
    ...
    const num1 = parseInt(data.compareNumber1?.toString() || "0");
    const num2 = parseInt(data.compareNumber2?.toString() || "0");
    
    setLeftAssets(Array.from({ length: num1 }, () => 
      SHAPE_ASSETS[Math.floor(Math.random() * SHAPE_ASSETS.length)]
    ));
    setRightAssets(Array.from({ length: num2 }, () => 
      SHAPE_ASSETS[Math.floor(Math.random() * SHAPE_ASSETS.length)]
    ));
  ...
  const analyzeWithAI = async () => {
    ...
    const imageDescription = `Group A has ${leftAssets.length} items and Group B has ${rightAssets.length} items.`;

    try {
      const feedback = await aiService.getFeedback(
        `Compare Group A and Group B. Which one is ${correctOption}?`,
        options,
        correctOption,
        attemptedOption,
        imageDescription,
      ...
      navigate('/video-lesson', { state: {
        ...
        fromPage: 'compare',
        question: `Compare A (${leftAssets.length}) and B (${rightAssets.length})`,
        correctOption,
        attemptedOption,
        leftAssets,
        rightAssets,
  ...
  if (showPretestIntro) return (
    <div className="min-h-screen bg-[#F5FFFF] flex flex-col items-center justify-center p-6 text-center">
      ...
      <h1 className="text-4xl font-bold text-[#6541EF] mb-4">
        {isHindi ? 'तुलना परीक्षण' : 'Compare Pre-test'}
      </h1>
      <p className="text-xl text-gray-600 mb-10 max-w-md">
        {isHindi 
          ? 'आइए देखें कि आप वस्तुओं की तुलना कितनी अच्छी तरह कर सकते हैं।' 
          : 'Let\'s see how well you can compare different quantities.'}
      </p>
      <button 
      ...
      <main className="p-4 max-w-4xl mx-auto">
        <div className="bg-[#6541EF] text-white px-6 py-4 rounded-2xl flex justify-between items-center mb-6 shadow-lg">
          <div>
            <span className="text-xs opacity-80 uppercase font-bold">
              {isPretestMode ? 'Compare (Pre-test)' : `Compare - ${currentQuestion.level}`}
            </span>
        ...
        <div className="bg-white rounded-[40px] p-8 shadow-xl border-t-8 border-[#6541EF]">
          <h2 className="text-2xl font-bold text-gray-800 mb-8 text-center">
            {isHindi ? 'सही तुलना चुनें' : 'Choose the correct comparison'}
          </h2>

          {/* Comparison View */}
          <div className="flex justify-between items-stretch gap-4 mb-10">
            <div className="flex-1 bg-blue-50 p-6 rounded-3xl border-2 border-blue-100 flex flex-col items-center">
              <span className="font-bold text-blue-600 mb-4 uppercase">Group A</span>
              <div className="flex flex-wrap gap-2 justify-center">
                {leftAssets.map((asset, i) => (
                  <img key={i} src={`/assets/${asset}`} alt="A" className="w-10 h-10 drop-shadow-sm" />
                ))}
              </div>
              <span className="mt-4 text-2xl font-black text-blue-700">{leftAssets.length}</span>
            </div>

            <div className="flex items-center">
               <div className="w-12 h-12 bg-[#6541EF] text-white rounded-full flex items-center justify-center font-bold italic">VS</div>
            </div>

            <div className="flex-1 bg-orange-50 p-6 rounded-3xl border-2 border-orange-100 flex flex-col items-center">
              <span className="font-bold text-orange-600 mb-4 uppercase">Group B</span>
              <div className="flex flex-wrap gap-2 justify-center">
                {rightAssets.map((asset, i) => (
                  <img key={i} src={`/assets/${asset}`} alt="B" className="w-10 h-10 drop-shadow-sm" />
                ))}
              </div>
              <span className="mt-4 text-2xl font-black text-orange-700">{rightAssets.length}</span>
            </div>
          </div>
            ...
            {currentQuestion.data.options.map((opt: any, idx: number) => {
              ...
              return (
                <button
                  ...
                  className={`p-5 rounded-2xl border-4 transition-all flex items-center justify-between group ${borderColor} ${bgColor}`}
                  disabled={hasSubmitted}
                >
                  <span className={`text-xl font-bold ${isSelected ? 'text-[#6541EF]' : 'text-gray-700'}`}>
                    {opt.title}
                  </span>
                  <VoiceIcon text={opt.title} isHindi={isHindi} size={18} />
                </button>
              );
            })}
          </div>
          ...
          <div className="mt-12 flex flex-col items-center gap-6">
            {!hasSubmitted ? (
              <button 
                ...
                className="w-full max-w-sm py-5 bg-[#6541EF] text-white rounded-2xl font-bold text-xl shadow-xl disabled:opacity-50"
              >
              ...
              <div className="flex flex-col w-full gap-4 items-center">
                {!currentQuestion.data.options[pendingSelectedIndex!].isCorrect && (
                   <button 
                     ...
                     className="flex items-center gap-2 text-[#6541EF] font-bold"
                   >
                     <Brain size={20} /> {isHindi ? 'AI से व्याख्या पूछें' : 'ASK AI FOR EXPLANATION'}
                   </button>
                )}
                ...
                <div className="flex gap-4 w-full max-w-sm">
                   <button 
                    onClick={handleNext}
                    className="flex-1 py-5 bg-[#6541EF] text-white rounded-2xl font-bold text-xl shadow-xl flex items-center justify-center gap-2"
                  >
