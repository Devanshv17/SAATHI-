...
import { collection, query, getDocs, orderBy, doc, limit, where, documentId } from 'firebase/firestore';
import { ref, get, update, increment, set as dbSet } from 'firebase/database';
import { aiService } from '../../services/aiService';
import { Brain, ArrowRight, X, Grid } from 'lucide-react';
...
export const Matching: React.FC<MatchingProps> = ({ gameTitle, isHindi }) => {
  const { user } = useAuth();
  ...
  if (showPretestIntro) return (
    <div className="min-h-screen bg-[#F5FFFF] flex flex-col items-center justify-center p-6 text-center">
       <div className="w-32 h-32 bg-white rounded-full flex items-center justify-center shadow-lg mb-8 border-4 border-[#6541EF]">
        <Grid size={64} className="text-[#6541EF]" />
      </div>
      <h1 className="text-4xl font-bold text-[#6541EF] mb-4">
        {isHindi ? 'मिलान परीक्षण' : 'Matching Test'}
      </h1>
      <p className="text-xl text-gray-600 mb-10 max-w-md">
        {isHindi 
          ? 'आइए देखें कि आप वस्तुओं का कितनी अच्छी तरह मिलान कर सकते हैं।' 
          : 'Let\'s see how well you can match different items.'}
      </p>
      <button 
      ...
      <main className="p-4 max-w-4xl mx-auto">
        <div className="bg-[#6541EF] text-white px-6 py-4 rounded-2xl flex justify-between items-center mb-6 shadow-lg">
          <div>
            <span className="text-xs opacity-80 uppercase font-bold">
              {isPretestMode ? 'Matching (Pre)' : `Level - ${currentQuestion.level}`}
            </span>
        ...
        <div className="bg-white rounded-[40px] p-8 shadow-xl border-t-8 border-[#6541EF]">
          <div className="flex flex-col items-center mb-10">
            <h2 className="text-2xl font-bold text-gray-800 mb-4 text-center leading-relaxed">
              {currentQuestion.data.text}
            </h2>
             {/* If it's Name Picture Matching, there might be an image at the top */}
            {currentQuestion.data.imageUrl && (
              <img src={currentQuestion.data.imageUrl} alt="Ref" className="w-48 h-48 object-contain rounded-2xl" />
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {currentQuestion.data.options.map((opt: any, idx: number) => {
              ...
              if (isSelected) {
                borderColor = "border-[#6541EF]";
                bgColor = "bg-[#F5F3FF] text-[#6541EF]";
              }
              if (showResult) {
                borderColor = isCorrect ? "border-green-500" : "border-red-500";
                bgColor = isCorrect ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700";
              }
              ...
              return (
                <button
                  ...
                  className={`p-6 rounded-[32px] border-4 transition-all flex items-center justify-between group h-full ${borderColor} ${bgColor}`}
                  disabled={hasSubmitted}
                >
                  <div className="flex items-center gap-4">
                    {opt.imageUrl && (
                      <img src={opt.imageUrl} alt={opt.title} className="w-16 h-16 object-contain rounded-xl" />
                    )}
                    <span className="text-xl font-black">{opt.title}</span>
                  </div>
                  <VoiceIcon text={opt.title} isHindi={isHindi} size={20} />
                </button>
              );
            })}
          </div>
              ...
              <div className="flex flex-col w-full gap-4 items-center">
                {!currentQuestion.data.options[pendingSelectedIndex!].isCorrect && (
                   <button 
                     ...
                     className="flex items-center gap-2 text-[#6541EF] font-bold py-2 px-4 rounded-xl"
                   >
                     {isAiAnalyzing ? (
