...
import { Brain, ArrowRight, ArrowLeft, Trophy, X } from 'lucide-react';

interface LetUsCountProps {
  gameTitle: string;
  isHindi: boolean;
}
...
export const LetUsCount: React.FC<LetUsCountProps> = ({ gameTitle, isHindi }) => {
  const { user } = useAuth();
  ...
  const assetsByQuestion = useRef<Record<string, string[]>>({});
  ...
  const initQuestionState = (qs: any[], idx: number) => {
    ...
    if (!assetsByQuestion.current[qId]) {
      const count = parseInt(data.numberField?.toString() || "0");
      assetsByQuestion.current[qId] = Array.from({ length: count }, () => 
        SHAPE_ASSETS[Math.floor(Math.random() * SHAPE_ASSETS.length)]
      );
    }
    setImageAssets(assetsByQuestion.current[qId]);

    if (userAnswers[qId]) {
      setPendingSelectedIndex(userAnswers[qId].selectedOptionIndex);
  ...
  const updateMainGameState = async (answers: Record<string, any>, isCorrect: boolean) => {
    ...
    const updates: any = {
      ...
      [`main_game/gameVisits/${dateKey}`]: increment(Math.floor((Date.now() - questionStartTime.current)/1000))
    };
  ...
  const analyzeWithAI = async () => {
    ...
    const imageDescription = `There are ${imageAssets.length} objects clearly visible in the image grid.`;
  ...
  if (showPretestIntro) return (
    <div className="min-h-screen bg-[#F5FFFF] flex flex-col items-center justify-center p-6 text-center">
      <div className="w-32 h-32 bg-white rounded-full flex items-center justify-center shadow-lg mb-8 border-4 border-[#6541EF]">
        <Trophy size={64} className="text-[#6541EF]" />
      </div>
      <h1 className="text-4xl font-trocchi font-bold text-[#6541EF] mb-4">
        {isHindi ? 'प्रारंभिक परीक्षण' : 'Pre-test'}
      </h1>
      <p className="text-xl text-gray-600 mb-10 max-w-md leading-relaxed">
        {isHindi 
          ? 'आइए देखें कि आप पहले से क्या जानते हैं। यह हमें आपके लिए सही स्तर चुनने में मदद करेगा।' 
          : 'Let\'s see what you already know. This will help us choose the right level for you.'}
      </p>
      <button 
      ...
      <main className="p-4 max-w-4xl mx-auto">
        ...
        <div className="bg-[#6541EF] text-white px-6 py-4 rounded-2xl flex justify-between items-center mb-6 shadow-lg">
          <div className="flex flex-col">
            <span className="text-xs opacity-80 uppercase tracking-widest font-bold">
              {isPretestMode ? (isHindi ? 'प्रारंभिक परीक्षण' : 'Pre-test') : (isHindi ? 'साक्षी: स्तर' : 'Saathi: Level')}
            </span>
            <h1 className="text-xl font-bold uppercase truncate max-w-[200px]">
              {isPretestMode ? gameTitle : `${gameTitle} - ${currentQuestion.level}`}
            </h1>
          </div>
        ...
        <div className="bg-white rounded-[40px] p-8 shadow-xl border-t-8 border-[#6541EF]">
          <div className="flex justify-between items-start mb-8">
            <h2 className="text-2xl font-bold text-gray-800 flex-1 leading-snug">
              {currentQuestion.data.text}
            </h2>
          </div>

          {/* Visual Assets Grid */}
          <div className="bg-[#BFEBF7] bg-opacity-20 rounded-3xl p-8 mb-8 border-2 border-dashed border-[#BFEBF7] min-h-[200px] flex items-center justify-center">
            <div className="flex flex-wrap gap-4 justify-center max-w-lg">
              {imageAssets.map((asset, i) => (
                <img 
                  ...
                  src={`/assets/${asset}`} 
                  alt="shape" 
                  className="w-16 h-16 drop-shadow-md animate-bounce"
            ...
            {currentQuestion.data.options.map((opt: any, idx: number) => {
              ...
              return (
                <button
                  ...
                  className={`p-6 rounded-[24px] border-4 transition-all flex items-center justify-between group shadow-sm hover:shadow-md ${borderColor} ${bgColor}`}
                >
                  <span className={`text-2xl font-bold ${isSelected ? 'text-[#6541EF]' : 'text-gray-700'}`}>
                    {opt.title}
                  </span>
                  <div className={`p-2 rounded-full transition-colors ${isSelected ? 'bg-[#6541EF] text-white' : 'bg-gray-100 text-gray-400 group-hover:bg-[#BFEBF7]'}`}>
                    <VoiceIcon text={opt.title} isHindi={isHindi} size={20} color="currentColor" />
                  </div>
                </button>
              );
            })}
          ...
          <div className="mt-12 flex flex-col items-center gap-6">
            {!hasSubmitted ? (
              <button 
                ...
                className="w-full max-w-sm py-5 bg-[#6541EF] text-white rounded-2xl font-bold text-xl shadow-xl hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 disabled:hover:scale-100"
              >
              ...
              <div className="flex flex-col w-full gap-4 items-center">
                {!currentQuestion.data.options[pendingSelectedIndex!].isCorrect && (
                   <button 
                     ...
                     className="flex items-center gap-3 text-[#6541EF] font-bold hover:underline mb-2"
                   >
                     {isAiAnalyzing ? (
                       <span className="flex items-center gap-2 animate-pulse">
                         <Brain size={24} /> Analyzing...
                       </span>
                     ) : (
                       <><Brain size={24} /> {isHindi ? 'AI से पूछें' : 'ASK AI FOR HELP'}</>
                     )}
                   </button>
                )}
                ...
                <div className="flex gap-4 w-full max-w-sm">
                   {currentQuestionIndex > 0 && (
                     <button 
                       onClick={() => {
                         ...
                         initQuestionState(questions, prev);
                       }}
                       className="p-4 bg-gray-100 text-gray-600 rounded-2xl font-bold border-2 border-transparent hover:border-gray-300 transition-all"
                     >
                       <ArrowLeft size={24} />
                     </button>
                   )}
                   <button 
                    onClick={handleNext}
                    className="flex-1 py-5 bg-[#6541EF] text-white rounded-2xl font-bold text-lg shadow-xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2"
                  >
                    {currentQuestionIndex < questions.length - 1 ? (
                      <>NEXT QUESTION <ArrowRight size={20} /></>
                    ) : (
                      <>FINISH LEVEL <ArrowRight size={20} /></>
                    )}
                  </button>
                </div>
              </div>
